...
class BaseStepper extends StatefulWidget {
  ...
  BaseStepper({
    ...
    assert(
        title!.length == subTitle!.length && title!.length == children!.length,
        'Error:children length and title and subtitle length dosent match');
  }
                    ...
                    children: <Widget>[
                      Stack(
                        ...
                        children: [
                          ...
                          Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: _customizedIndicator(index),
                          ),
                        ],
                      ),
                      SizedBox(
  ...
  Widget _customizedDottedLine(int index, Axis axis) {
    return index < widget.children!.length - 1
        ? DottedLine(
            ...
            spacing: 3.0,
            axis: axis,
            // stepperCount: widget.children!.length,
            // stepperSize: widget.lineDotRadius,
          )
        : Container();
  }
