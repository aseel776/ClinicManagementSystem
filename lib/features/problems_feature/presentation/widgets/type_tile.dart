...
class ProblemTypeTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    bool editing = ref.watch(editingProblemType(key));
    return Container(
      height: tileHeight,
      width: tileWidth,
      ...
      child: editing
          ? TextFieldTapRegion(
              onTapOutside: (event) {
                ref.read(editingProblemType(key).notifier).state = false;
                ...
                children: [
                  Expanded(
                    child: TextFormField(
                      ...
                      decoration: InputDecoration(
                        ...
                        contentPadding:
                            EdgeInsets.only(bottom: -tileHeight * .2),
                      ),
                      validator: (value) {
                      ...
                      onFieldSubmitted: (value) async {
                        if (value.isEmpty) {
                          ...
                          ref.read(editingProblemType(key).notifier).state = false;
                          await ref.read(problemTypesProvider.notifier).updateProblemType(type);
                        }
                      },
                    ),
                  ),
                  SizedBox(
                    ...
                    child: FloatingActionButton(
                      onPressed: () async {
                        ref.read(editingProblemType(key).notifier).state = false;
                        await ref.read(problemTypesProvider.notifier).deleteProblemType(type.id!);
                      },
                ...
                SizedBox(
                  ...
                  child: FloatingActionButton(
                    onPressed: () {
                      ...
                      ref.read(editingProblemType(key).notifier).state = true;
                    },
                    shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(5),
                    ),
